...
999999{0}", Today.ToString("ddMMyyyy"));
        ...
        [Fact]
        public async Task Clientes_Caracteristicas_Teste()
        {
            var filename = $"{nameof(Clientes_Caracteristicas_Teste)}.erp";
            var conteudo = new StringBuilder();

            conteudo.AppendLine(Arquivo.GetHeader);
            conteudo.AppendLine(Clientes.GetHeader);

            conteudo.AppendLine(RetornaClientesCaracteristicas.First());
...
999999{0}", Today.ToString("ddMMyyyy"));
        ...
        [Fact]
        public async Task Clientes_Pessoa_Relacionada_Teste()
        {
            var filename = $"{nameof(Clientes_Pessoa_Relacionada_Teste)}.erp";
            var conteudo = new StringBuilder();

            conteudo.AppendLine(Arquivo.GetHeader);
            conteudo.AppendLine(Clientes.GetHeader);

            conteudo.AppendLine(RetornaClientesPessoaRelacionada.First());
...
999999{0}", Today.ToString("ddMMyyyy"));
        ...
        public async Task Tres_Clientes_Todos_Dados_Por_Cliente_Teste()
        {
            ...
            conteudo.Append(new Clientes
            {
                ...
                ClientesCaracteristicas = RetornaClientesCaracteristicas.First(),
                ClientesPessoaRelacionada = RetornaClientesPessoaRelacionada.First(),
